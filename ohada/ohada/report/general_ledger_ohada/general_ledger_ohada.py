...
from ohada.ohada.report.financial_statements_ohada import get_cost_centers_with_children
...
def get_gl_entries(filters, accounting_dimensions):
	...
	select_fields = """, debit * taux_reference as debit, credit * taux_reference as credit, debit_in_account_currency * taux_reference as debit_in_account_currency,
		credit_in_account_currency * taux_reference as credit_in_account_currency """
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			cost_center, project, taux_reference,
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		balance = get_balance(d, balance, "debit_in_account_currency", "credit_in_account_currency")
		d["balance"] = balance
...
def get_columns(filters):
	if filters.get("presentation_currency"):
		...
		if filters.get("company"):
			currency = frappe.db.get_value("Company", filters["company"], "reference_currency", cache=True)
		else:
			company = get_default_company()
			currency = frappe.db.get_value("Company", company, "reference_currency", cache=True)
