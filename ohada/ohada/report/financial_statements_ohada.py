...
def get_data(
	...
	company_currency = frappe.db.get_value("Company", company, "reference_currency", cache=True)
			...
			for period in period_list:
				...
				if entry.posting_date <= period.to_date:
					if (accumulated_values or entry.posting_date >= period.from_date) and (
						...
						d[period.key] = d.get(period.key, 0.0) + flt(entry.debit_in_account_currency * entry.taux_reference) - flt(entry.credit_in_account_currency * entry.taux_reference)

			if entry.posting_date < period_list[0].year_start_date:
				d["opening_balance"] = d.get("opening_balance", 0.0) + flt(entry.debit_in_account_currency * entry.taux_reference) - flt(entry.credit_in_account_currency * entry.taux_reference)
	...
	if accounts:
		...
		gl_entries = frappe.db.sql(
			...
			select posting_date, account, debit, credit, is_opening, fiscal_year,
				debit_in_account_currency, credit_in_account_currency, account_currency, taux_reference from `tabGL Entry`
